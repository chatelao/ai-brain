            ...
            <main>
                <div class="px-4 pt-6">
                    ...
                    <div class="mb-6" x-data="{
                        task: <?= htmlspecialchars(json_encode($taskModel->getTaskUiInfo($task))) ?>
                    }" @sync-updated.window="
                        if ($event.detail.tasks) {
                            const updated = $event.detail.tasks.find(t => t.task_id === task.task_id);
                            if (updated) task = { ...task, ...updated };
                        }
                    ">
                        <div class="flex flex-wrap items-center justify-between gap-4 mb-4">
                            ...
                            <div class="flex items-center space-x-2">
                                <span class="px-4 py-1.5 text-sm font-semibold rounded-full flex items-center shadow-sm"
                                      :class="'bg-' + task.status_color + '-100 text-' + task.status_color + '-800'">
                                    <span x-text="task.status_emoji + ' ' + task.status_label"></span>
                                </span>
                            </div>
                        </div>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="flex items-center justify-between">
                                        ...
                                        <span class="px-2.5 py-0.5 rounded-full text-xs font-medium"
                                              :class="{
                                                  'bg-purple-100 text-purple-800': task.github_state === 'closed',
                                                  'bg-green-100 text-green-800': task.github_state === 'open'
                                              }"
                                              x-text="task.github_state.charAt(0).toUpperCase() + task.github_state.slice(1)">
                                        </span>
                                    </div>

                                    <!-- Jules Session -->
                                    <div class="flex items-center justify-between">
                                        <template x-if="task.jules_url">
                                            <a :href="task.jules_url" target="_blank" rel="noopener noreferrer" class="flex items-center text-sm text-purple-600 hover:underline">
                                        ...
                                        <span class="px-2.5 py-0.5 rounded-full text-xs font-medium"
                                              :class="{
                                                  'bg-gray-100 text-gray-800': task.jules_color === 'gray',
                                                  'bg-yellow-100 text-yellow-800': task.jules_color === 'yellow',
                                                  'bg-green-100 text-green-800': task.jules_color === 'green',
                                                  'bg-red-100 text-red-800': task.jules_color === 'red'
                                              }"
                                              x-text="task.jules_display_status">
                                        </span>
                                    </div>

                                    <!-- Pull Request -->
                                    <div class="flex items-center justify-between">
                                        <template x-if="task.pr_url">
                                            <a :href="task.pr_url" target="_blank" rel="noopener noreferrer" class="flex items-center text-sm hover:underline"
                                               :class="'text-' + (task.pr_color === 'gray' ? 'gray' : task.pr_color) + '-600'">
                                        ...
                                        <span class="px-2.5 py-0.5 rounded-full text-xs font-medium"
                                              :class="{
                                                  'bg-gray-100 text-gray-800': task.pr_color === 'gray',
                                                  'bg-green-100 text-green-800': task.pr_color === 'green',
                                                  'bg-purple-100 text-purple-800': task.pr_color === 'purple',
                                                  'bg-red-100 text-red-800': task.pr_color === 'red'
                                              }"
                                              x-text="task.pr_status">
                                        </span>
                                    </div>
                                </div>

...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_telegram_config'])) {
    if (!$auth->validateCsrfToken($_POST['csrf_token'] ?? null)) {
        die("CSRF token validation failed.");
    }
            ...
            <main>
                <div class="px-4 pt-6">
                    ...
                    <div class="mb-4">
                        ...
                        <p class="text-sm text-gray-500 mt-1">Manage your API keys and external account connections.</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                        <div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
                            <h4 class="text-lg font-semibold text-gray-900 mb-2">Jules API Key (Private)</h4>
                            <p class="text-sm text-gray-600 mb-4">Your personal Google AI (Gemini) API key. This is required for agent features and is not shared with other users.</p>
                        ...
                        <div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
                            <h4 class="text-lg font-semibold text-gray-900 mb-2">Telegram Custom Bot</h4>
                            <p class="text-sm text-gray-600 mb-4">Optional: Use your own Telegram Bot. Configure the webhook to:<br>
                                <code class="bg-gray-100 px-1 rounded text-xs font-mono">https://<?= $_SERVER['HTTP_HOST'] ?? 'your-domain.com' ?>/telegram-webhook.php?user_id=<?= $user['user_id'] ?></code>
                            </p>
